...
하나라도 만족하지 못하면 해당 회차를 `INVALID`로 처리한다. runner는 실패한 요청의 번호·시나리오·HTTP 상태 코드 또는 예외 종류를 보고하고, 공식 결과 파일을 생성하지 않는다. 해당 회차의 부분 측정값은 p50·p95 계산에 사용하지 않는다. 실패 원인을 확인한 뒤 새로운 SQLite DB에서 회차 전체를 다시 실행한다.
...
정상 실행 조건을 모두 만족한 회차에서만 공식 결과 파일을 생성하고 다음 정보를 보존한다.
